...
svgEl.addEventListener("mousemove", (e) => {
    ...
    if (grabbing) {
        ...
        if (dataCloseToMouse.length > 0)
            ...
            svgEl.setAttribute("style", "cursor: default");
    }
...
for (let x = chartDataBounds.xAxisMin; x <= chartDataBounds.xAxisMax; x += X_AXIS_TICK_DATA_DISTANCE) {
...
for (let y = chartDataBounds.yAxisMin; y <= chartDataBounds.yAxisMax; y += Y_AXIS_TICK_DATA_DISTANCE) {
...
function convertPointBetweenRanges(sourceMin, sourceMax, targetMin, targetMax, v) {
...
export function svgPointFromDatumPoint([x, y]) {
    return [
        convertPointBetweenRanges(chartDataBounds.xAxisMin, chartDataBounds.xAxisMax, chartSvgBounds.left, chartSvgBounds.right, x),
        convertPointBetweenRanges(chartDataBounds.yAxisMin, chartDataBounds.yAxisMax, chartSvgBounds.bottom, chartSvgBounds.top, y),
    ];
}
export function datumPointFromSvgPoint([x, y]) {
    return [
        convertPointBetweenRanges(chartSvgBounds.left, chartSvgBounds.right, chartDataBounds.xAxisMin, chartDataBounds.xAxisMax, x),
        convertPointBetweenRanges(chartSvgBounds.bottom, chartSvgBounds.top, chartDataBounds.yAxisMin, chartDataBounds.yAxisMax, y),
    ];
}
/*****************
...
function makeCircleSvgEl(svgEl) {
